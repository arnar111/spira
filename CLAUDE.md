...
- `season.ts` — Reykjavík **outdoor** season/frost calendar (last frost ~late May, first frost late Sep); `frostRisk()` / `seasonForMonth()` / `seasonStatus()` drive outdoor advice (planting window, hilling, harvest-before-frost). The Rós engine branches on `growIsOutdoor()`: outdoor grows skip the indoor watering/feed/LED/hand-pollination insights and get season/frost/hilling/mulch ones instead.
